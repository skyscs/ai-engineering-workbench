...
import { DomainError, parseRepositoryInput, type GitFailure, type Repository } from '@aew/core';
...
export class RepositoryService {
  ...
  async startClone(workspaceId: string, value: unknown): Promise<Repository> {
    ...
    if (this.stopping || this.jobs.size) throw new DomainError('CONFLICT', 'Another clone is active or the daemon is shutting down. Wait before starting another clone.');
    const id = randomUUID();
