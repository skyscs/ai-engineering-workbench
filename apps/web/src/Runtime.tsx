...
import type { RunEvent, RuntimeDetail, StageRun, TaskDetail, WorkspaceDetail } from '@aew/shared';
import { api } from './api';

export function Runtime({ detail, settings, csrf, disabled, update }: {
  detail: TaskDetail; settings: WorkspaceDetail; csrf: string | null; disabled: boolean; update(value: TaskDetail): void;
}) {
  const base = `/workspaces/${detail.task.workspaceId}/tasks/${detail.task.id}`;
  const connection = settings.connection, profiles = settings.modelProfiles;
  ...
  async function start() {
    ...
    try {
      await api<StageRun>(`${base}/investigations`, { method: 'POST', headers, body: JSON.stringify({ modelProfileId: profile || null }) });
  ...
  return <section aria-label="AI runtime" className="runtime-section"><h3>Historical investigation</h3>
    ...
    <button disabled={disabled || busy || active || !connection?.configHome || detail.worktrees.some((r) => r.status !== 'ready')} onClick={() => void start()}>Run investigation</button>
    {error && <p role="alert" className="error">{error}</p>}
    {latest && <><p>Run {latest.id}: {run?.run.status ?? latest.status}</p>
      <p>Recorded configuration directory: <code>{latest.inputSnapshot.connection.configHome ?? 'Not recorded by the earlier application version'}</code></p>
