...
const defaultConnection: ConnectionInput = { name: 'Codex connection', executablePath: null, configProfile: null };

export function ConnectionFields({ value, onChange, locked = false }: {
  value: ConnectionInput; onChange: (value: ConnectionInput) => void; locked?: boolean;
}) {
  return <>
    ...
    <label>CLI configuration<select name="configMode" disabled={locked} value={value.configProfile === null ? 'default' : 'named'}
      ...
      <option value="default">Current CLI configuration</option><option value="named">Named CLI profile</option>
    </select></label>
...
export function ConnectionForm({ initial, locked, busy, onSave }: {
  ...
  const [value, setValue] = useState<ConnectionInput>({ name: initial.name,
    executablePath: initial.executablePath, configProfile: initial.configProfile });
  return <form aria-label="Edit connection" onSubmit={(event) => { event.preventDefault(); void onSave(value); }}>
    <fieldset disabled={busy}>
      <ConnectionFields value={value} onChange={setValue} locked={locked} />
      {locked && <p className="hint">This data boundary is locked. Create another workspace to change the CLI configuration.</p>}
